...
router.post("/", tempUserCreationLimiter({ maxCreations: 10, windowMs: 30 * 60 * 1000 }), createTempUser);

// GET /api/temp-users/:tempId - Get temporary user by ID
router.get("/:tempId", requireAuth, requireOwnership((req) => req.params.tempId), getTempUserById);

// POST /api/temp-users/:tempId/goals - Add a goal to temporary user
router.post("/:tempId/goals", requireAuth, requireOwnership((req) => req.params.tempId), addGoalToTempUser);
